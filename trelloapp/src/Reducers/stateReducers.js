...
function stateReducers(state = intialState, action) {
  ...
  switch (action.type) {
    ...
    case "get_card":
      if (
        stateCopy.list[action.payload.index].boardId ===
        action.payload.data.boardId
      ) {
        stateCopy.list[action.payload.index].cards = action.payload.data.cards;
      }
      // if (stateCopy.list[action.payload.index]._id === action.payload.listId) {
      // stateCopy.list[action.payload.index].cards  = [
      //     ...stateCopy.list[action.payload.index].cards,action.payload.data];
      // }
      return stateCopy;
    ...
    case "Draggable_Happen":
      ...
      if (droppableIdStart !== droppableIdEnd) {
        ...
        let listStart = stateCopy.list.find(
          (list) => droppableIdStart === list._id
        );
        ...
        let listEnd = stateCopy.list.find(
          (list) => droppableIdEnd === list._id
        );
    ...
    case "Edit_Card":
      ...
      stateCopy.editData = stateCopy.list.find(
        (el) => el._id === action.payload.listId
    ...
    case "Delete_List":
      ...
      stateCopy.list = stateCopy.list.filter(
        (el) => el._id !== action.payload.listId
      );
